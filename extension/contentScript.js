...
function cleanPage() {
  ...
  const distractingSelectors = [
    ...
    'iframe', '.newsletter', '.subscription', '.popup',
    '.cookie-notice', '.gdpr', '.notification'
  ];
  ...
  if (!mainContent) {
    ...
    paragraphs.forEach(p => {
      ...
      while (parent && parent !== doc.body) {
        if (!textBlocks[parent.tagName]) {
          textBlocks[parent.tagName] = { element: parent, textLength: 0 };
        }
        textBlocks[parent.tagName].textLength += p.textContent.length;
    ...
    for (const tag in textBlocks) {
      if (textBlocks[tag].textLength > maxTextLength) {
        maxTextLength = textBlocks[tag].textLength;
        contentElement = textBlocks[tag].element;
      }
    }
...
function createOrUpdateSidebar(readTime) {
  ...
  if (currentView === 'clean') {
    ...
    if (summary) {
      ...
      contentArea.innerHTML = '<p>No summary available. Click "Summarize" to generate one.</p>';
...
function setViewMode(mode) {
  ...
  if (mode === 'raw') {
    ...
    if (document.body.classList.contains('cleanread-active')) {
      document.body.innerHTML = originalContent;
      document.body.classList.remove('cleanread-active');
    }
  } else {
    // Show sidebar
    if (sidebar) {
    ...
    // Clean the page if not already cleaned
    if (!cleanedContent) {
      cleanPage();
    }
  }
    ...
    (response) => {
      if (response && response.summary) {
        ...
        if (sidebar) {
          const contentArea = sidebar.querySelector('div:nth-child(4)');
          if (contentArea && currentView === 'summary') {
            contentArea.innerHTML = '<p>Failed to generate summary. Please try again.</p>';
          }
        }
      }
    }
